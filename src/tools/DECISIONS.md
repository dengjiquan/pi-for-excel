...
- **UTF-8 enforcement on child process (`runPython`):** the bridge launches Python with the `-X utf8` CLI flag (alongside `-I` for isolation). `-X utf8` forces UTF-8 mode (stdin/stdout/stderr + `open()` defaults) so `print()`/`result` payloads never fall back to the system locale. NOTE: an earlier attempt set `PYTHONUTF8`/`PYTHONIOENCODING` on the child env, but `-I` (isolated mode) silently ignores ALL `PYTHON*` env vars, so those had zero effect and stdout still defaulted to cp936/GBK on zh-CN Windows (verified: mojibake → `U+FFFD` at the Node decode boundary). The `-X utf8` CLI flag is the correct mechanism because it is NOT ignored by `-I`. This is the encoding-contract fix at the channel layer; skills no longer need per-skill `sys.stdout.reconfigure()` workarounds.
