...
if (useHttps && useHttp) {
  console.error(
    "[pi-for-excel] Invalid args: can't use both --https and --http",
  );
  process.exit(1);
}
...
if (!MODE) {
  console.error(
    `[pi-for-excel] Invalid PYTHON_BRIDGE_MODE: ${MODE_RAW}. Use "stub" or "real".`,
  );
  process.exit(1);
}
...
function probeBinary(command, args) {
  ...
  if (probe.error) {
    ...
    console.warn(
      `[pi-for-excel] Binary "${command}" not available (${code}): ${message}`,
    );
...
async function runPython(request, pythonInfo) {
  ...
  const result = await runCommandCapture({
    ...
    // -X utf8 forces UTF-8 stdout/stderr so print(json.dumps(ensure_ascii=False))
    // never falls back to the Windows locale (cp936/GBK). MUST be a -X flag, NOT
    // a PYTHONUTF8/PYTHONIOENCODING env var: -I isolated mode drops env-based
    // encoding overrides, and Node decodes our pipe as UTF-8 regardless.
    // Without it, Chinese in Access query results comes back as mojibake.
...
const server = (() => {
  ...
  if (!fs.existsSync(keyPath) || !fs.existsSync(certPath)) {
    console.error(
      "[pi-for-excel] HTTPS requested but key.pem/cert.pem not found in repo root.",
    );
    console.error(
...
server.listen(PORT, HOST, () => {
  ...
  console.log(
    `[pi-for-excel] python bridge listening on ${scheme}://${HOST}:${PORT}`,
  );
  console.log(`[pi-for-excel] mode: ${backend.mode}`);
  console.log(`[pi-for-excel] health: ${scheme}://${HOST}:${PORT}/health`);
  console.log(
    `[pi-for-excel] endpoint: ${scheme}://${HOST}:${PORT}/v1/python-run`,
  );
  console.log(
    `[pi-for-excel] endpoint: ${scheme}://${HOST}:${PORT}/v1/libreoffice-convert`,
  );
  console.log(
    `[pi-for-excel] allowed origins: ${Array.from(allowedOrigins).join(", ")}`,
  );

  if (authToken) {
    console.log(
      "[pi-for-excel] auth: bearer token required for POST endpoints",
    );
  } else {
    console.warn(
      "[pi-for-excel] auth: no bearer token configured — any local process on this machine can drive this bridge.",
    );
    console.warn(
      "[pi-for-excel] recommended: set PYTHON_BRIDGE_TOKEN=<secret> and mirror it in Pi via /experimental python-bridge-token <secret>",
    );
  }

  if (backend.mode === "stub") {
    console.log(
      "[pi-for-excel] stub mode: python/libreoffice calls are simulated.",
    );
    console.log(
      "[pi-for-excel] use PYTHON_BRIDGE_MODE=real for local command execution.",
    );
  }
});
